...
export default function Signup() {
  ...
  async function signup(e) {
    ...
    try {
      await fetch(
        `${API}/auth/signup?name=${form.get("name")}&email=${form.get("email")}&password=${form.get("password")}`,
        { method: "POST" }
      );
      ...
      >
        <div className="text-center mb-10">
          ...
          <div className="flex items-center justify-center gap-3 opacity-60">
            ...
            <p className="text-cyan-400 text-[10px] tracking-[0.2em] uppercase font-mono">Initialize New Terminal Session</p>
            <div className="h-[1px] w-12 bg-gradient-to-l from-transparent to-cyan-500/50"></div>
          </div>
        </div>

        <form ref={formRef} onSubmit={signup} className="space-y-6">
